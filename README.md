...
- **提示词**：按模型版本匹配专属格式（V5 自然语言+标签自由混合；V4/V4.5 标签为主+数字权重；V3 纯 Danbooru 标签）。**V5 另集成 nai5-prompting 深度方法**：`references/通用构思.md`（构思层）+ `references/通用写法.md`（写法层，词组/句子实测判据、多角色绑定、漫画分格、检查表），来源 [github.com/Miint-Sunny/nai5-prompting](https://github.com/Miint-Sunny/nai5-prompting)（GPL-3.0）；脚本模式的适配规则见 SKILL.md「V5 桥接要点」
- **直连 API 生成**：不依赖页面 UI；自动解包 ZIP、随机/固定 seed、429 并发锁指数退避重试、生成台账 `manifest.jsonl`
- **生成前标签核对**：`scripts/check-tag.mjs` 每次生成前只读查询 danbooru（写法存在性 / post 量 / 类别，具名角色必查），按"名字直触 / 中间档 / 保守"三档落笔，减少角色设定偏差
- **安全内置**：生图仅允许 novelai.net 系主机；标签核对仅只读 `danbooru.donmai.us`（不携带任何凭据）；DNS 解析后拒绝内网/保留地址、不打印令牌
...
- 脚本只向 `image.novelai.net` / `api.novelai.net` 发请求，并校验 DNS 解析结果不是内网地址；`check-tag.mjs` 只读 `danbooru.donmai.us` 的 tags.json（不带凭据，UA 固定 `curl/8.7.1` 通过 Cloudflare 挑战）。
